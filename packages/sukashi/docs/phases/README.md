...
**Estado actual:** F0 · F1 · F2 · F3 · F4 · F4½ · F5 · F6 · F7 · **F8** ✅ — plan completo; F8 = extensión warifu (motivo sellado por el manifiesto de hanko), núcleo + demo + e2e (ver [`STATUS.md`](../STATUS.md)).
...
| **F8** | (extensión) Tablilla partida (warifu) | un motivo (y bytes) que solo se compone cuando un manifiesto cumple su condición; adaptador al Trust Report de hanko + demo | ✅ hecha |
...
**Hecho cuando:** (incr.1) núcleo puro testeable en node — `match` correcto revela (error 0), `match` equivocado / `tally` sola = ruido, bytes round-trip, adaptador hanko abre con report todo-sellado y no abre si algo queda sin sellar. (incr.2) demo «Candado · tablilla» con selector de manifiesto (Proyecto sellado → abre · Trust Report real hoy → cerrada · Casi → cerrada) + e2e contra un snapshot real del Trust Report (`demo/fixtures/hanko-trust-report.json`). ✅ 14 tests (`src/warifu/warifu.test.ts`).
