...
Siguiente: ninguna fase del **camino crítico** pendiente (F0→F6 completas). Opcional enchufable restante: **F4½** (deformación uzumaki). **F7** (semilla del cielo / kumo) ✅ hecha.

> Plan de obra completo (F0→F7) en [`phases/README.md`](phases/README.md) · visual en [`phases/roadmap.html`](phases/roadmap.html).
> Detalle de F4 en [`patterns.md`](patterns.md) · reporte de contraste de F6 en [`contrast-report.html`](contrast-report.html) · detalle de F7 en [`entropy.md`](entropy.md).
