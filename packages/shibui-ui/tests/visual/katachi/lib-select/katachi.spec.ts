/**
 * lib-select · Katachi visual regression
 * ───────────────────────────────────────
 * Checks the select trigger, placeholder text, chevron, and selected-value
 * rendering across all 6 katachi contexts.
 *
...
test.describe('lib-select · katachi visual', () => {
  ...
  for (const id of KATACHI) {
    test(`slice · ${id}`, async ({ page }) => {
      ...
      await waitForComponents(page, 'lib-select', 'lib-select-option');

      const slice = page.locator(`[data-slice="${id}"]`);
      await expect(slice).toBeVisible();

      await expect(slice).toHaveScreenshot(`lib-select-${id}.png`, {
  ...
  test('grid · all 6 katachi', async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot('lib-select-grid.png', {
      fullPage: true,
      maxDiffPixelRatio: 0.01,
      animations: 'disabled',
    });
