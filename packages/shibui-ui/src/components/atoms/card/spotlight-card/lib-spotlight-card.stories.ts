...
type LibSpotlightCardStoryArgs = Pick<LibSpotlightCard, 'spotlight' | 'gold'>;
...
const meta: Meta<LibSpotlightCardStoryArgs> = {
  ...
  argTypes: {
    spotlight: {
      control: 'select',
      options: ['accent', 'water', 'white'],
      description: 'Color del foco de luz reactivo al cursor',
    },
    gold: {
      control: 'boolean',
      description: 'Activa el hilo de oro en el borde (--lib-gold-border)',
    },
  },
  ...
  render: (args): TemplateResult => stage(1, html`
    ...
    >
      ${cardInner(
        `--lib-spotlight-gradient${args.spotlight !== 'accent' ? '-' + args.spotlight : ''}`,
        ...
        `oklch(${args.spotlight === 'accent' ? '45% 0.05 45' : args.spotlight === 'water' ? '55% 0.06 210' : '100% 0 0'} / 0.12)`
      )}
    </lib-spotlight-card>
  `),
};
...
export const SpotlightVariants: Story = {
  render: (): TemplateResult => stage(3, html`
    <lib-spotlight-card spotlight="accent">
      ${cardInner(
...
export const KintsugiVariant: Story = {
  ...
  render: (): TemplateResult => html`
    <div style="background:#0f1923; padding:48px; border-radius:8px;">
      <lib-spotlight-card gold style="max-width:100%;">
        <div style="padding:24px; display:grid; grid-template-columns:1fr 1fr; gap:32px; align-items:center;">
          <div>
            <p style="font-family:monospace; font-size:10px; letter-spacing:0.25em; text-transform:uppercase; color:oklch(60% 0 0); margin-bottom:12px;">--lib-gold-border</p>
            ...
            <p style="font-size:13px; color:oklch(60% 0.01 220); line-height:1.8;">
              El hilo de oro en las grietas. Gradiente diagonal en el borde que evoca el arte japonés de reparar cerámica con oro. Spotlight accent activo al mover el cursor.
            </p>
          </div>
...
export const Comparison: Story = {
  ...
  render: (): TemplateResult => stage(2, html`
    <lib-spotlight-card spotlight="accent">
      ${cardInner('Sin gold', 'Spotlight puro', 'Fondo neutro oscuro. Solo el foco de luz reacciona al cursor.', '--lib-spotlight-gradient')}
    </lib-spotlight-card>

    <lib-spotlight-card spotlight="accent" ?gold=${true}>
      ${cardInner('Con gold', 'Spotlight + Border', 'Fondo cálido oscuro. Hilo de oro en el borde y foco reactivo.', '--lib-gold-border')}
    </lib-spotlight-card>
  `),
};
  ...
  render: (): TemplateResult => stage(3, html`
    ...
    <lib-spotlight-card ?gold=${true}>
      <div style="padding:24px;">
        ...
        <p style="font-size:12px; color:oklch(55% 0.01 220); line-height:1.8;">Glass, spotlight y gold border como tokens opcionales del sistema.</p>
      </div>
    </lib-spotlight-card>
  `),
};
...
/* ═══════════════════════════════════════════════════════════════
   ...
   lib-spotlight-card usa --lib-spotlight-* y --lib-gold-border
   effect tokens. El bg-base del contenedor adapta el contraste
   del foco reactivo al cursor según el katachi activo.
   ═══════════════════════════════════════════════════════════════ */
  ...
  <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:var(--lib-space-sm);max-width:600px;">
    <lib-spotlight-card spotlight="accent">
      <div style="padding:var(--lib-space-md);">
        <p style="font-family:var(--lib-font-mono);font-size:9px;letter-spacing:.2em;text-transform:uppercase;color:var(--text-muted);margin-bottom:var(--lib-space-xs);">accent</p>
    ...
    <lib-spotlight-card spotlight="white" gold>
      <div style="padding:var(--lib-space-md);">
        <p style="font-family:var(--lib-font-mono);font-size:9px;letter-spacing:.2em;text-transform:uppercase;color:var(--text-muted);margin-bottom:var(--lib-space-xs);">white · gold</p>
