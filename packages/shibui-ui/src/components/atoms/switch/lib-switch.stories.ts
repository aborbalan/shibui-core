...
type SwitchArgs = Pick<LibSwitch, 'checked' | 'disabled' | 'surface' | 'size' | 'label' | 'sub'>;
...
const meta: Meta<SwitchArgs> = {
  ...
  render: (args): TemplateResult => html`
    <div style="padding:40px; background:${args.surface === 'inverse' ? 'var(--color-washi-950)' : 'var(--bg-surface)'}; border:1px solid var(--border-subtle);">
      <lib-switch
...
export const KintsugiStates: Story = {
  ...
  render: (): TemplateResult => html`
    <div style="padding:40px; background:var(--color-washi-950); border:1px solid oklch(16% 0.02 45); display:flex; flex-direction:column; gap:24px;">
      <lib-switch surface="inverse" label="Cerámica oscura" sub="Off — venas sutiles"></lib-switch>
      <lib-switch surface="inverse" label="Thumb dorado" sub="On — halo pulsante" ?checked=${true}></lib-switch>
      <lib-switch surface="inverse" label="Exportación avanzada" sub="Requiere plan Enterprise" ?disabled=${true}></lib-switch>
      <lib-switch surface="inverse" label="Disabled activado" ?checked=${true} ?disabled=${true}></lib-switch>
    </div>
  `,
};
...
export const KintsugiSizes: Story = {
  ...
  render: (): TemplateResult => html`
    <div style="padding:40px; background:var(--color-washi-950); border:1px solid oklch(16% 0.02 45); display:flex; align-items:center; gap:48px; flex-wrap:wrap;">
      ${(['sm', 'md', 'lg'] as const).map(size => html`
        <div style="display:flex; flex-direction:column; align-items:center; gap:16px;">
          <lib-switch surface="inverse" size=${size} ?checked=${true}></lib-switch>
          <lib-switch surface="inverse" size=${size}></lib-switch>
...
export const KintsugiLabel: Story = {
  ...
  render: (): TemplateResult => html`
    <div style="padding:40px; background:var(--color-washi-950); border:1px solid oklch(16% 0.02 45); display:flex; flex-direction:column; gap:24px; max-width:480px;">
      <lib-switch surface="inverse" label="Acceso premium" sub="Funciones exclusivas desbloqueadas" ?checked=${true}></lib-switch>
      <lib-switch surface="inverse" label="Modo ceremonial" sub="Activa el tema inverse en toda la interfaz"></lib-switch>
      <lib-switch surface="inverse" label="Sincronización de perfil" sub="Última actualización · hace 2 min" ?checked=${true}></lib-switch>
      <lib-switch surface="inverse" label="Exportación avanzada" sub="Requiere plan Enterprise" ?disabled=${true}></lib-switch>
    </div>
  `,
};
