    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:var(--lib-space-lg);padding:var(--lib-space-xl);background:var(--color-washi-100);">
      ${katachiList.map(k => html`
        <section data-katachi="${k.id}" style="padding:var(--lib-space-lg);display:flex;flex-direction:column;gap:var(--lib-space-md);background:var(--bg-base);">
          ...
          <div style="min-height:80px;display:flex;align-items:center;gap:var(--lib-space-sm);">
            <button style="font-family:var(--lib-font-mono);font-size:11px;letter-spacing:.1em;text-transform:uppercase;padding:8px 16px;background:var(--bg-elevated);color:var(--text-secondary);border:1px solid var(--border-default);cursor:pointer;">Mostrar toast</button>
            <p style="font-size:var(--text-xs,10px);color:var(--text-muted);">Contexto ${k.id}</p>
          </div>
        </section>
      `)}
    </div>
