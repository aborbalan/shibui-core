...
- ✅ `specs/checks-contract.md` — contrato declarado ↔ runtime (F3, incr. 1 + incr. 2: faceta slot).
- ✅ `specs/checks-a11y.md` — verificación de accesibilidad universal: axe + teclado/foco/nombre (F4).
- ✅ `specs/checks-resilience.md` — resiliencia universal: props basura/vacías, RTL, SSR (F5).
- ✅ `specs/harness.md` — harness de runtime que observa elementos vivos (incr. 2 de F3/F4/F5; validado en navegador).
- ✅ `specs/trust-report.md` — Trust Report: agregador del sello (Floor+contrato+a11y+resiliencia) + renderers JSON/HTML (F6).
- ✅ `specs/packaging.md` — desacople (guard de genericidad ejecutable) + paquete publicable (F7); primer `npm publish` manual pendiente.
...
- ✅ `phases/` — índice F0–F7 + tracker visual `development-phases.html` (F0–F7 terminadas).
